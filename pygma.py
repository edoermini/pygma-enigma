...
class m4():
	def __init__(self):
		...
		self.rotor_4 = ["a:z", "b:u", "c:i", "d:q", "e:v", "f:w", "g:x", "h:p", "i:l", "j:k", "k:d", "l:r", "m:n", "n:j", "o:t", "p:y", "q:s", "r:f", "s:m", "t:h", "u:a", "v:g", "w:o", "x:e", "y:c", "z:b"]
		self.rotor_4_left = []
		self.rotor_4_right = []
	...
	def set_key(self, key):
		...
		if (len(key) > 5 or len(key) < 5):
			raise ValueError("The encryption key must be 5 letters\n")
		...
		for k in key:
			...
			try:
				if (count == 1):
					...
					index = self.rotor_4_left.index(k)
					self.rotor_4_left = self.rotor_4_left[index:] + self.rotor_4_left[:index]
					self.rotor_4_right = self.rotor_4_right[index:] + self.rotor_4_right[:index]
	...
	def enc_dec(self, word):
		...
		rotor_3_left_mv = self.rotor_3_left
		rotor_3_right_mv = self.rotor_3_right
		rotor_4_left_mv = self.rotor_3_left
		rotor_4_right_mv = self.rotor_3_right
		reflector_left_mv = self.reflector_left
		reflector_right_mv = self.reflector_right
		...
		for i in word:
			...
			try:
				...
				if rotor_1_count == 26:
					...
					rotor_4_left_mv = rotor_4_left_mv[1:] + rotor_4_left_mv[:1]
					rotor_4_right_mv = rotor_4_right_mv[1:] + rotor_4_right_mv[:1]
